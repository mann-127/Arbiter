...
# In-memory storage — protected by an asyncio lock to prevent concurrent write races
...
def get_recent_incidents(hours: int = Query(24, ge=1, le=720, description="Hours to look back")) -> List[Incident]:
...
async def record_routing_decision(decision: RoutingDecisionInput) -> RoutingDecision:
...
async def record_feedback(feedback: FeedbackInput) -> FeedbackRecord:
    """Record outcome feedback for a routing decision."""
    prediction_error = feedback.predicted_latency_ms - feedback.actual_latency_ms
    was_correct = (feedback.actual_latency_ms > 300 and "REROUTE" in feedback.routing_decision) or (
        feedback.actual_latency_ms <= 300 and "REROUTE" not in feedback.routing_decision
    )
...
def get_feedback_stats(window_minutes: int = Query(60, ge=5, le=1440)) -> Dict[str, float]:
    """Get aggregate feedback statistics over a time window."""
    if not feedback_store:
        return {"total_records": 0, "mae": 0.0, "accuracy": 0.0, "rmse": 0.0}
