...
class ChaosSimulator:
    ...
    Use either `apply_chaos()` (which advances time) or `step()` (which
    advances time without applying metrics effects) — never both in the same
    simulation loop, as each call increments `current_step`.
    """
    ...
    def inject_failure(
        ...
        if affected_backends is None:
            affected_backends = [self.backends[0]]
    ...
    def apply_chaos(self, base_metrics: Dict) -> Dict:
        """Apply active chaos to metrics and advance simulation time by one step.

        Note: This method advances `current_step`. Do NOT also call `step()` in
        the same loop iteration — that would double-count and end chaos early.
        ...
        chaos_start_step = self.chaos_end_step - chaos.duration_steps

        if chaos.event_type == FailureType.LATENCY_SPIKE:
            multiplier = 1 + (chaos.intensity * 5)
            ...
            progress = (self.current_step - chaos_start_step) / chaos.duration_steps
            metrics["error_rate"] = min(0.9, progress * chaos.intensity)
            metrics["avg_latency_ms"] *= 1 + progress * 4
            metrics["current_load"] *= 1 + progress
            ...
            if random.random() < chaos.intensity:
                ...
                metrics["avg_latency_ms"] *= 2

        return metrics

    def step(self) -> None:
        """Advance simulation time by one step without applying metrics effects.

        Use this only when you do NOT call apply_chaos() in the same iteration.
        Calling both step() and apply_chaos() in the same loop tick will
        advance current_step twice, causing chaos to end one step early.
        """
        self.current_step += 1
        if self.active_chaos and self.current_step >= self.chaos_end_step:
            logger.info(f"Chaos ended: {self.active_chaos.description}")
...
class ResilienceTest:
    ...
    def __init__(self, router_func: Callable):
        ...
        Args:
            router_func: Function that takes metrics and returns routing decision string
        """
    ...
    def run_scenario(
        ...
        Args:
            ...
            normal_steps: Steps of normal operation before chaos
        ...
        for step in range(total_steps):
            ...
            # Remember whether chaos was active BEFORE apply_chaos() advances the step,
            # so the final chaos step is correctly classified as a chaos step.
            was_chaos_active = self.chaos.is_active()
            ...
            logger.info(
                f"T+{step}: Load={metrics['current_load']:.0f}, "
        ...
        result = {
            ...
            "chaos_detection_rate": reroutes_during_chaos / len(decisions_during_chaos)
            if decisions_during_chaos
            else 0,
            "false_positive_rate": reroutes_during_normal / len(decisions_during_normal)
            if decisions_during_normal
            else 0,
            "total_reroutes": reroutes_during_chaos + reroutes_during_normal,
            "passed": reroutes_during_chaos >= len(decisions_during_chaos) * 0.7,
        }

        self.results.append(result)

        status = "PASSED" if result["passed"] else "FAILED"
        logger.info(f"{status}: Detection rate = {result['chaos_detection_rate'] * 100:.0f}%")
    ...
    def get_summary(self) -> Dict:
        ...
        return {
            ...
            "avg_detection_rate": float(np.mean([r["chaos_detection_rate"] for r in self.results])),
            "avg_false_positive_rate": float(np.mean([r["false_positive_rate"] for r in self.results])),
        }
