...
class AnomalyDetector:
    ...
    def __init__(self, contamination: float = 0.05, window_size: int = 100, warmup_samples: int = 50):
    ...
    def _generate_description(self, metrics: Dict, score: float) -> str:
        ...
        if metrics.get("latency_slope", 0) > 50:
            parts.append("Rapid latency increase")
    ...
    def update(self, metrics: Dict) -> Optional[Dict]:
        ...
        Returns:
            Dict with anomaly details once past warmup, None during warmup
        """
        ...
        # Threshold-based rules complement IsolationForest for obvious spikes.
        threshold_anomaly = (
        ...
        if is_anomaly:
            ...
            if severity == "high":
                ...
                logger.warning(f"MEDIUM ANOMALY: {description}")
            else:
    ...
    def get_recent_anomalies(self, n: int = 10) -> List[AnomalyEvent]:
        """Get recent AnomalyEvent objects."""
        return self.anomaly_history[-n:]

    def get_anomaly_history(self) -> List[Dict]:
        """Return full detection history (all samples, anomalies and normal)."""
    ...
    def get_status(self) -> Dict:
        ...
        return {
            ...
            "recent_anomalies": len(self.anomaly_history[-100:]),
        }
...
if __name__ == "__main__":
    ...
    for i in range(50):
        ...
        if result and result.get("is_anomaly"):
            print(f"\nT+{i}: [{result['severity'].upper()}] {result['description']}")
