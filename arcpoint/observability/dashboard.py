...
class ContextServiceClient:
    ...
    def get_feedback_stats(self, window_minutes=60):
        ...
        try:
            resp = self.session.get(
                f"{self.base_url}/feedback/stats", params={"window_minutes": window_minutes}, timeout=2
            )
            return resp.json() if resp.status_code == 200 else {}
        except Exception as e:
    ...
    def get_recent_decisions(self, limit=10):
        ...
        try:
            resp = self.session.get(f"{self.base_url}/decisions/recent", params={"limit": limit}, timeout=2)
            return resp.json() if resp.status_code == 200 else []
        except Exception as e:
...
with col1:
    ...
    st.metric("Current Load", f"{current_load} req/min")

with col2:
    mae = feedback_stats.get("mae", 0)
    st.metric("Prediction MAE", f"{mae:.1f} ms", f"Last {window_minutes}m")
...
# Trend indicator — API returns "up", "down", or "stable"
trend = traffic_forecast.get("trend", "stable")
predicted_rpm = traffic_forecast.get("predicted_requests_per_min", 0)
if trend == "up":
    st.info(f"📈 Traffic **increasing** — Predicted: {predicted_rpm} req/min")
elif trend == "down":
    st.warning(f"📉 Traffic **decreasing** — Predicted: {predicted_rpm} req/min")
else:
    st.success(f"📊 Traffic **stable** — Predicted: {predicted_rpm} req/min")
...
with col_right:
    ...
    if backend_status:
        ...
        fig = px.bar(backend_df, x="backend_id", y="utilization", title="")
