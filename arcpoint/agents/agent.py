...
from .prompts import ROUTING_QUERY_TEMPLATE
...
class RoutingAgent:
    ...
    def __init__(self, use_mock: bool = True):
        ...
        Args:
            use_mock: If True, use mock LLM responses (no API calls).
                      If False, an LLM client must be wired up in make_routing_decision().
        """
        self.context_api = ContextAPI()
        self.use_mock = use_mock

        if not use_mock:
            logger.info("Real LLM API mode selected — wire up client in make_routing_decision()")
        else:
    ...
    def _format_context(self, user_id: str) -> str:
        ...
        incidents_str = (
            "\n".join([f"- [{i['severity'].upper()}] {i['affected_service']}: {i['description']}" for i in incidents])
            or "None"
        )
    ...
    def _mock_llm_response(self, context: str) -> Dict:
        ...
        return {
            ...
            "reasoning": (
                ...
                f"and high availability. Routing to {best_backend} which has "
                f"{backends[0]['current_load'] / backends[0]['capacity'] * 100:.0f}% utilization."
            ),
            "confidence": "high",
    ...
    def make_routing_decision(self, user_id: str) -> Dict:
        ...
        Raises:
            NotImplementedError: When use_mock=False and no LLM client is wired up
        """
        ...
        if self.use_mock:
            ...
            # Wire up a real LLM client here before using in production:
            # import anthropic
