...
class FeedbackCollector:
    ...
    def record(
        ...
        # Correctness is ground-truth only: did we make the right call given what actually happened?
    ...
    def get_metrics(self) -> Dict:
        """Get summary metrics."""
        if not self.records:
            return {"total": 0, "accuracy": 0.0, "mae": 0.0}
...
class OnlineLearner:
    ...
    @staticmethod
    def _make_model() -> SGDRegressor:
        """Construct SGDRegressor with standard hyperparameters."""
        return SGDRegressor(
            loss="squared_error",
            penalty="l2",
            alpha=0.001,
    ...
    def reset(self) -> None:
        """Reset learner state after drift or rollback, preserving lifetime sample count."""
        samples_seen = self.samples_seen
        self.scaler = StandardScaler()
        self.model = self._make_model()
        self.is_fitted = False
        self.samples_seen = samples_seen  # preserve lifetime count for observability
        self.feature_buffer = []
        self.target_buffer = []
...
class DriftDetector:
    ...
    def update(self, error: float) -> bool:
        ...
        # Capture mean before updating so the PH statistic uses the prior estimate
        prev_mean = self.mean
        self.mean = self.mean + (error - self.mean) / self.n

        # Page-Hinkley statistic using the pre-update mean
        self.sum = self.sum + (error - prev_mean - self.alpha)
        ...
        if self.n >= self.min_samples and ph_value > self.threshold:
            if not self.drift_detected:
                ...
                logger.warning(f"Drift detected at sample {self.n}! PH value: {ph_value:.2f}")
            return True
...
class ABTestFramework:
    ...
    def __init__(self, control_ratio: float = 0.9):
        """Initialize A/B test framework.

        Args:
            control_ratio: Proportion of traffic to control (default 90%)
        """
        self.control_ratio = control_ratio
        self.control_weight = control_ratio  # alias kept for internal callers
    ...
    def start_test(self, control_ratio: float = 0.9) -> None:
        """Start a new A/B test."""
        self.control_ratio = control_ratio
        self.control_weight = control_ratio
        self.control_outcomes = []
        self.treatment_outcomes = []
        self.is_active = True
        logger.info(f"A/B test started: {control_ratio * 100:.0f}% control, {(1 - control_ratio) * 100:.0f}% treatment")
    ...
    def assign_variant(self, request_id: str) -> Literal["control", "treatment"]:
        ...
        bucket: Literal["control", "treatment"] = "control" if hash_value < self.control_ratio * 100 else "treatment"
    ...
    def get_results(self) -> Dict:
        ...
        t_stat = (control_mean - treatment_mean) / se if se > 0 else 0.0

        significant = abs(t_stat) > 1.96  # 95% confidence

        improvement = (control_mean - treatment_mean) / control_mean * 100 if control_mean > 0 else 0.0
    ...
    def conclude_test(self) -> str:
        ...
        self.is_active = False
        recommendation = results.get("recommendation", "Keep control")
        logger.info(f"A/B test concluded: {recommendation}")
